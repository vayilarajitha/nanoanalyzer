...
try {
    ...
    // Insert into nanoparticle_datasets
    try {
        $ds_stmt = $pdo->prepare("INSERT INTO nanoparticle_datasets 
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $ds_stmt->execute([
            $uuid, $user_id, $analysis_name, $analysis_name, $shape, $shape, $material, $material, $charge, $charge, $nanoparticle_size, $nanoparticle_size, $concentration, $cell_type, $uptake_percentage, $predicted_toxicity, $optimization_recommendation
        ]);
