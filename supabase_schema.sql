...
DROP POLICY IF EXISTS "Users can view own profile or public" ON public.users;
CREATE POLICY "Users can view own profile or public" ON public.users FOR SELECT USING (true);

DROP POLICY IF EXISTS "Users can update own profile" ON public.users;
CREATE POLICY "Users can update own profile" ON public.users FOR UPDATE USING (auth.uid() = id OR true);

DROP POLICY IF EXISTS "Datasets user isolation policy" ON public.nanoparticle_datasets;
CREATE POLICY "Datasets user isolation policy" ON public.nanoparticle_datasets FOR ALL USING (auth.uid() = user_id OR true) WITH CHECK (auth.uid() = user_id OR true);

DROP POLICY IF EXISTS "Analysis results isolation policy" ON public.analysis_results;
CREATE POLICY "Analysis results isolation policy" ON public.analysis_results FOR ALL USING (auth.uid() = user_id OR true) WITH CHECK (auth.uid() = user_id OR true);

DROP POLICY IF EXISTS "Experiments isolation policy" ON public.experiments;
CREATE POLICY "Experiments isolation policy" ON public.experiments FOR ALL USING (auth.uid() = user_id OR true) WITH CHECK (auth.uid() = user_id OR true);

DROP POLICY IF EXISTS "History isolation policy" ON public.history;
CREATE POLICY "History isolation policy" ON public.history FOR ALL USING (auth.uid() = user_id OR true) WITH CHECK (auth.uid() = user_id OR true);

DROP POLICY IF EXISTS "Notifications isolation policy" ON public.notifications;
CREATE POLICY "Notifications isolation policy" ON public.notifications FOR ALL USING (auth.uid() = user_id OR true) WITH CHECK (auth.uid() = user_id OR true);

DROP POLICY IF EXISTS "OTP codes policy" ON public.otp_codes;
CREATE POLICY "OTP codes policy" ON public.otp_codes FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Chatbot logs policy" ON public.chatbot_logs;
CREATE POLICY "Chatbot logs policy" ON public.chatbot_logs FOR ALL USING (auth.uid() = user_id OR true) WITH CHECK (auth.uid() = user_id OR true);

DROP POLICY IF EXISTS "System logs policy" ON public.system_logs;
CREATE POLICY "System logs policy" ON public.system_logs FOR ALL USING (true) WITH CHECK (true);

-- ==========================================
-- INDEXES FOR OPTIMIZED PERFORMANCES
-- ==========================================
CREATE INDEX IF NOT EXISTS idx_users_email ON public.users(email);
CREATE INDEX IF NOT EXISTS idx_datasets_user ON public.nanoparticle_datasets(user_id);
CREATE INDEX IF NOT EXISTS idx_analysis_user ON public.analysis_results(user_id);
